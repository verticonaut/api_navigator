...
describe ApiNavigator do
  describe 'new' do
    it 'creates a new EntryPoint with the url' do
      ApiNavigator::EntryPoint.expects(:new).with('http://api.example.org')

      ApiNavigator.new('http://api.example.org')
    end

    describe 'with an optional block' do
      let(:client) do
        ApiNavigator.new('http://api.example.org') do |client|
